...
if isphi
  cx = simpson_summation(u(1,:));  % Compute x-integration constant
  cy = simpson_summation(v(:,1));  % Compute y-integration constant
  phi = simpson_summation(v)+cx(ones(ly,1),:);
  phi = (phi+simpson_summation(u')'+cy(:,ones(1,lx)))/2;
end

 % Compute streamfunction PSI (solenoidal part)
if ispsi
  cx = simpson_summation(v(1,:));  % Compute x-integration constant
  cy = simpson_summation(u(:,1));  % Compute y-integration constant
  psi = -simpson_summation(u)+cx(ones(ly,1),:);
  psi = (psi+simpson_summation(v')'-cy(:,ones(1,lx)))/2;
end
